...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/stock/stock': { kind: 'indirect', reason: 'The service interface delegates model rendering to dsh-tool-stock; it registers no prompt section or tool schema of its own.' },
  ...
  'packages/client/ui-watchlist': { kind: 'none', reason: 'Browser-side global panel over the watchlist and stock Remotes; it registers no prompt, tool, or session event.' },
  ...
  'packages/api/stock-controller': { kind: 'indirect', reason: 'Browser-facing market-data reads; dsh-tool-stock owns every model-facing projection of the same seam.' },
  'packages/api/watchlist-controller': { kind: 'none', reason: 'Durable personal instrument list behind a Remote namespace; it registers no prompt, tool, or session event.' },
